...
import { useTemplates, type TemplateComponentInput, type TemplateButtonInput } from '@/hooks/use-templates';
...
function makeButton(type: EditButton['type']): EditButton {
  return { type, text: '', url: '', phone_number: '', example: '' };
}

function getButtonsForEdit(components: unknown): EditButton[] {
  if (!Array.isArray(components)) return [];
  const comps = components as Array<Record<string, unknown>>;
  const btnComp = comps.find((c) => c['type'] === 'BUTTONS');
  if (!btnComp || !Array.isArray(btnComp['buttons'])) return [];
  return (btnComp['buttons'] as Array<Record<string, unknown>>).map((b) => ({
    type: (b['type'] as EditButton['type']) || 'QUICK_REPLY',
...
export default function EditTemplatePage() {
  ...
  const existingButtons = useMemo(
    () => (template ? getButtonsForEdit(template.components) : []),
    [template],
  );

  const initialButtonGroup = useMemo((): 'QUICK_REPLY' | 'CTA' => {
    if (existingButtons.length === 0) return 'QUICK_REPLY';
    return existingButtons[0].type === 'QUICK_REPLY' ? 'QUICK_REPLY' : 'CTA';
  }, [existingButtons]);
  ...
  const existingHeaderText = template ? getHeaderText(template.components) : '';
  const existingBodyText = template ? getBodyText(template.components) : '';
  const existingFooterText = template ? getFooterText(template.components) : '';
  const existingBodyExamples = template ? getBodyExamples(template.components) : [];

  const [headerEnabled, setHeaderEnabled] = useState(existingHeaderText !== '');
  const [headerText, setHeaderText] = useState(existingHeaderText);
  const [bodyText, setBodyText] = useState(existingBodyText);
  const [bodySamples, setBodySamples] = useState<string[]>(existingBodyExamples);
  const [footerEnabled, setFooterEnabled] = useState(existingFooterText !== '');
  const [footerText, setFooterText] = useState(existingFooterText);

  const [buttonsEnabled, setButtonsEnabled] = useState(existingButtons.length > 0);
  const [buttonGroup, setButtonGroup] = useState<'QUICK_REPLY' | 'CTA'>(initialButtonGroup);
  const [buttons, setButtons] = useState<EditButton[]>(existingButtons);
  ...
  const addButton = (type: EditButton['type']) => setButtons((prev) => [...prev, makeButton(type)]);
  const updateButton = (index: number, patch: Partial<EditButton>) =>
  ...
  const isValid = bodyText.trim() !== '' && samplesValid && buttonsValid;
  ...
  if (!template) {
    return (
      <div className="flex flex-col h-full items-center justify-center">
        <p className="text-[14px]" style={{ color: '#6b7671' }}>Template not found.</p>
        <button
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <span className="text-[16px] font-[650] tracking-tight font-mono" style={{ color: '#1b2420' }}>
          {template.name}
        </span>
      </div>
        ...
        <div className="grid gap-5 h-full" style={{ gridTemplateColumns: 'minmax(0, 1fr) 340px' }}>
          ...
          <div className="bg-card border border-border rounded-[14px] px-6 py-5 overflow-y-auto">
            ...
            <FieldGroup label="Template Name">
              <div
                className="w-full px-[11px] py-2 border border-border rounded-[8px] font-mono text-[13px]"
                style={{ background: '#f5f6f3', color: '#6b7671' }}
              >
                {template.name}
              </div>
