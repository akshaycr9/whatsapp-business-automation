...
export const TemplatePreviewModal = React.memo(function TemplatePreviewModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[500] flex items-center justify-center p-6 animate-fade-in bg-black/45"
      onClick={onClose}
    >
      <div
        className="bg-card border border-border rounded-[16px] max-w-[380px] w-full flex flex-col overflow-hidden animate-scale-in shadow-lg-cf"
        style={{
      ...
      >
        ...
        <div className="flex items-start justify-between px-5 pt-[18px] pb-3.5 border-b border-border">
          <div>
            <div className="font-mono text-[15px] font-bold text-ink-900">
              {template.name}
            </div>
            <div className="flex items-center gap-2 mt-1.5 text-[12px] text-ink-500">
              <StatusBadge status={template.status} />
              <span>{template.category} · {template.language.toUpperCase()}</span>
            </div>
          </div>
          <button
            className="w-8 h-8 grid place-items-center rounded-[8px] transition-colors hover:bg-surface-sunken text-ink-700"
            onClick={onClose}
          >
            <X size={18} />
          </button>
        ...
        <div className="flex justify-end gap-2 px-5 py-3.5 border-t border-border">
          <button
            className="inline-flex items-center gap-1.5 px-3 py-[7px] rounded-[8px] border border-border bg-card text-[13px] font-semibold transition-colors hover:bg-surface-sunken text-ink-900"
            onClick={onClose}
          >
            Close
          </button>
          <button
            className="inline-flex items-center gap-1.5 px-3 py-[7px] rounded-[8px] text-[13px] font-semibold transition-colors bg-brand-700 text-white border border-brand-800 hover:bg-brand-800"
            onClick={onEdit}
          >
            <Settings size={13} />
            Edit Template
