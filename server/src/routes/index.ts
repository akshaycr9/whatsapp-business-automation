...
import razorpayWebhookRoutes from "./webhooks/razorpay.webhook.js";
...
export const registerRoutes = (app: Express): void => {
  ...
  app.post("/api/setup/register-webhooks", async (_req, res, next) => {
    try {
      if (!env.PUBLIC_URL) {
        res.status(400).json({
          error: {
            message: "PUBLIC_URL is not set in .env",
            statusCode: 400,
          },
        });
        return;
      ...
      res.json({
        ok: true,
        message: "Shopify webhooks registered successfully",
      });
    } catch (err) {
      next(err);
    }
  ...
  app.get("/api/debug/webhooks", (_req, res) => {
    ...
    res.json({
      ...
      ngrok_tip:
        "Run: ngrok http 3000 — use the https:// URL as PUBLIC_URL in .env",
      shopify: {
  ...
  app.use("/api/webhooks/rp-abandoned-cart", razorpayWebhookRoutes);
  ...
  app.use((req: Request, res: Response) => {
    logger.warn(
      `404 Not Found: ${req.method} ${req.path} — check webhook URL configuration`,
    );
    res.status(404).json({
      error: {
  ...
  if (env.PUBLIC_URL) {
    ...
    registerWebhooks(env.PUBLIC_URL).catch((err: unknown) => {
      ...
      logger.warn(
        "   ⚠️  Manual webhook setup may be required in Shopify Admin",
      );
    });
  } else {
    logger.warn(
      "   ⚠️  PUBLIC_URL not set — Shopify webhooks will NOT be auto-registered",
    );
    logger.warn(
      "   Set PUBLIC_URL=https://xxx.ngrok-free.dev in .env and restart",
    );
  }
  logger.info("─────────────────────────────────────────────────────");
