...
const automationSchema = automationBaseObject
  .refine((d) => d.categoryId !== undefined, {
...
// GET /api/automations — return automations grouped by category
router.get('/', async (_, res, next) => {
  try {
    const result = await automationService.listGroupedByCategory();
...
router.post('/', async (req, res, next) => {
  try {
    const body = automationSchema.parse(req.body) as automationService.CreateAutomationInput;
